...
{
    /// <summary>
    /// Handles the low-level protocol for communicating with the PLC, with a focus on performance and low memory allocations.
    /// </summary>
    public class PlcProtocol
    {
        ...
        /// <summary>
        /// Sends a packet to the PLC using pooled memory to avoid allocations.
        /// </summary>
        public async Task SendPacketAsync(byte[] contents, int step = 2, int sleepMs = 10, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < packetToSend.Length; i += step)
                {
                    ...
                    await _channel.WriteAsync(packetBuffer, i, bytesToSend, cancellationToken).ConfigureAwait(false);
                    if (sleepMs > 0)
                    {
        ...
        /// <summary>
        /// Receives a packet from the PLC using pooled memory to avoid allocations.
        /// </summary>
        public async Task<byte[]?> ReceivePacketAsync(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                await _channel.ReadAsync(lengthBuffer, 0, 1, cancellationToken).ConfigureAwait(false);
                ...
                try
                {
                    ...
                    {
                        cancellationToken.ThrowIfCancellationRequested();
                        bytesRead += await _channel.ReadAsync(fullPacketBuffer, 1 + bytesRead, bytesToRead - bytesRead, cancellationToken).ConfigureAwait(false);
                    }
                    ...
                    return ProtocolUtils.DecodePacket(fullPacketSpan);
                }
                finally
                {
