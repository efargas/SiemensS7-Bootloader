...
namespace S7.Core.Abstractions.Validation
{
    ...
    public sealed class HexAddressAttribute : ValidationAttribute
    {
        ...
        protected override ValidationResult? IsValid(object? value, ValidationContext validationContext)
        {
            ...
            {
                return AllowEmpty ? ValidationResult.Success : new ValidationResult(FormatErrorMessage(validationContext.DisplayName));
            }
            ...
            if (uint.TryParse(hexString[2..], NumberStyles.HexNumber, CultureInfo.InvariantCulture, out uint hexValue))
            {
                ...
                {
                    return new ValidationResult($"The {validationContext.DisplayName} field must be between 0x{MinValue:X} and 0x{MaxValue:X}.");
                }
            }
            else
            {
        ...
        protected override ValidationResult? IsValid(object? value, ValidationContext validationContext)
        {
            ...
            try
            {
                ...
                if (MustExist)
                {
                    ...
                    if (!exists)
                    {
                        return new ValidationResult($"The {validationContext.DisplayName} field must specify an existing {(AllowDirectories ? "file or directory" : "file")}.");
                    }
                }
                ...
                if (AllowedExtensions != null && AllowedExtensions.Length > 0)
                {
                    ...
                    {
                        return new ValidationResult($"The {validationContext.DisplayName} field must have one of the following extensions: {string.Join(", ", AllowedExtensions)}.");
                    }
                }
            ...
            {
                return new ValidationResult($"The {validationContext.DisplayName} field contains an invalid path: {ex.Message}");
            }
        }
    }
    ...
    public sealed class NetworkEndpointAttribute : ValidationAttribute
    {
        ...
        protected override ValidationResult? IsValid(object? value, ValidationContext validationContext)
        {
            ...
            if (parts.Length != 2)
            {
                return new ValidationResult($"The {validationContext.DisplayName} field must be in the format 'host:port'.");
            }
            ...
            {
                return new ValidationResult($"The {validationContext.DisplayName} field must specify a valid host.");
            }
            ...
            if (!isValidHost)
            {
                return new ValidationResult($"The {validationContext.DisplayName} field must specify a valid {(AllowIpAddresses && AllowHostnames ? "IP address or hostname" : AllowIpAddresses ? "IP address" : "hostname")}.");
            }

            // Validate port
            if (!int.TryParse(portString, out int port) || port < MinPort || port > MaxPort)
            {
                return new ValidationResult($"The {validationContext.DisplayName} field must specify a valid port number between {MinPort} and {MaxPort}.");
            }
    ...
    public sealed class NumericRangeAttribute : ValidationAttribute
    {
        ...
        protected override ValidationResult? IsValid(object? value, ValidationContext validationContext)
        {
            ...
            {
                return new ValidationResult($"The {validationContext.DisplayName} field must be a valid number.");
            }

            if (numericValue < MinValue || numericValue > MaxValue)
            {
                return new ValidationResult(string.Format(CultureInfo.InvariantCulture, ErrorMessage ?? "The {0} field must be between {1} and {2}.", validationContext.DisplayName, MinValue, MaxValue));
            }

            if (Alignment > 1 && numericValue % Alignment != 0)
            {
                return new ValidationResult($"The {validationContext.DisplayName} field must be aligned to {Alignment} (divisible by {Alignment}).");
            }
    ...
    public sealed class TimeoutAttribute : ValidationAttribute
    {
        ...
        protected override ValidationResult? IsValid(object? value, ValidationContext validationContext)
        {
            ...
            else
            {
                ...
                {
                    return new ValidationResult($"The {validationContext.DisplayName} field must be a valid timeout value.");
                }
            }

            if (timeoutMs < MinTimeoutMs || timeoutMs > MaxTimeoutMs)
            {
                return new ValidationResult(string.Format(CultureInfo.InvariantCulture, ErrorMessage ?? "The {0} field must be a valid timeout between {1}ms and {2}ms.", validationContext.DisplayName, MinTimeoutMs, MaxTimeoutMs));
            }
